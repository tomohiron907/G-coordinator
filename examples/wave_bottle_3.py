...
for height in range(LAYER):
    ...
    amp = 2
    rad = base_rad+amp*np.sin(arg*50.5+np.pi*height) + 5*np.sin((height+arg/(2*np.pi))/LAYER*np.pi*5)
    x = rad*np.cos(arg )
    y = rad*np.sin(arg )
    ...
    if height <2:
        ...
        bottom = gc.line_infill(inner_wall,  infill_distance = 1.2, angle = np.pi/4 + np.pi/2*height)
